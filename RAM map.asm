...
$51..86: Regular IO registers (according to $81:8D0F)
{
    ...
    $77: Display resolution ($2133). Updated during NMI
    {
        v = ex00gbsi
        i: Interlacing
...
$05B5: 8-bit frame counter. Does not include lag frames. Not sure why this exists...
...
$05CF: Debug. Debug options
{
    ...
    8000h: Ammo is swapped (B is pressed whilst select + R is held) ($80:9459)
}
...
$0619..28: Music queue. List of words. If negative, low byte is index to music data, else new music track (*2 = SPC 581E index)
$0629..38: Music queue timers. List of words. Minimum of 8
$0639: Music queue next index
$063B: Music queue start index
$063D: Music entry
{
    1: Samus fanfare
    2: Item fanfare
...
$064C: Current music track. FFh means new music data is being uploaded
...
$0727: Menu index
{
    ...
    File select menu:
    {
        0: Fade out and configure graphics for file select menu
        1: Load file select menu BG2
        2: Initialise main
        3: Fade in to main
        4: Main
        5: File copy - fade out to file copy
        6: File copy - initialise
        7: File copy - fade in to file copy
        ...
        Fh: File copy - fade out to main
        10h: File copy - reload main
        11h: File copy - fade in to main
        12h: File copy - menu index = 4
        13h: File clear - fade out to file clear
        14h: File clear - initialise
        15h: File clear - fade in to file clear
        ...
        1Bh: File clear - fade out to main
        1Ch: File clear - reload main
        1Dh: File clear - fade in to main
        1Eh: File clear - menu index = 4
        1Fh: Turn Samus helmet
        20h: Fade out to options menu
        21h: Fade out to title sequence
    }
    
    File select map:
    {
        0: Clear BG2 and set up fade out
        1: Fade out and load area palettes
        2: Load area select foreground tilemap
        3: Load area select background tilemap
        4: Prepare expanding square transition to area select map
        5: Expanding square transition to area select map
        6: Area select map
        7: Prepare expanding square transition to room select map
        8: Expanding square transition to room select map
        9: Initialise room select map
        Ah: Room select map
        Bh: 
        Ch: 
        Dh: 
...
$07F3: Music data index
$07F5: Music track index
...
$092D: Samus' speed during door transition
...
$0939:
{
    Proposed scrolled layer 1 X position in $80:A528. [layer 1 X position] ± ([X distance Samus moved + 1] + 2)
...
$0998: Game state
{
    ...
    13h: Samus ran out of health.                                                       Set by $82:DB69 (Samus ran out of health, no reserves)
    14h: Samus ran out of health, black out surroundings.                               Set by $82:DC80 (game state 13h)
    15h: Samus ran out of health, black out surroundings.                               Set by $82:DCE0 (game state 14h)
    16h: Samus ran out of health, starting death animation.                             Set by $82:DD71 (game state 15h)
    17h: Samus ran out of health, flashing.                                             Set by $82:DD87 (game state 16h)
    18h: Samus ran out of health, explosion.                                            Set by $82:DD9A (game state 17h)
    19h: Samus ran out of health, black out (also cut to by time up death).             Set by $82:8431 (whiting out from time up), $82:DDAF (game state 18h), $8B:C627
    ...
    28h: Transition to demo.                                                            Set by $82:85FB (transition from demo), $8B:9FAE
    29h: Transition to demo.                                                            Set by $82:8000 (game state 28h)
    2Ah: Playing demo.                                                                  Set by $82:852D (game state 29h)
    2Bh: Transition from demo.                                                          Set by $82:8548 (demos ended)
    2Ch: Transition from demo.                                                          Set by $82:8593 (game state 2Bh)
}
...
$099E: Menu option index (in options screen and submenus)
...
$09A2..0A01: Save to SRAM
{
    ...
    $09E6: Debug. 0: Allows positioning Samus in demo recorder, toggled by B on controller 2. Allows A on controller 2 to freeze time. Set to 1 by when loading demo data or making new save file after setting controller bindings etc., but seemingly for no reason
    $09E8: Set to 1 when loading demo data or making new save file after setting controller bindings etc.. Otherwise unused
    $09EA: HUD auto-cancel flag. 1 = on
...
$0A02..0E0B: Samus RAM (according to $91:E018)
{
    ...
    $0A1F: Samus movement type
    {
        ...
        Ch: Unused. Can fire grapple beam and charge pose. No pose definitions correspond to this
    ...
    $0A28: Potential new value for Samus' current pose. Controller-attempted move, taking Samus' previous pose, buttons, and momentum into account in that order.
    $0A2A: Potential new value for Samus' current pose? Interruption via 90DDE9; enemy damage or bomb jump, also grapple connection.
    $0A2C: Potential new value for Samus' current pose. Natural following of previous pose (generally used by transitional poses).
    $0A2E: Momentum routine index? JSR index for EC16,X, bank 91 (Cause of Yapping Maw Super Jump crash. I think. (918304 --> 91EBEE --> ...)
    $0A30: Special Samus transition graphics handler index? JSR index for EC28,X, bank 91. Related to 0A2A. 01 = hurt(18AA)? and DE82,X routine(X = 0A1F) does SEC (todo), 03 = hurt(0A52, !0A56)? and DFB5,X (X = 0A1F) does SEC (todo), 04 = unused code?, 05 = possibly set by 91E16D (unknown use), 09 and 0A = grapple related (also death related?)
    $0A32: JSR index for EC3E,X, bank 91. Related to 0A2C, source? 01 = hurt($0A2C = 29/2A if hurt movement, else $0A1C), 03 = animation loop transition, 02 = , 06 and 07 = death related, 08 = hurt during animation loop transition
    ...
    $0A46: If v & 2 != 0, enables $94:87F4 (Samus horizontal slope collision). Always 3. Set to 0/1 by unused(?) code ($91:F2F0) if collided with solid enemy and previous movement type = 9.
    ...
    $0A5A: Pointer to code to run (JMP from $90:E097) (Once every frame, depending on 7E0A44). Special instructions for things like when Ridley runs away from Ceres.
    {
        ...
        $E37E: RTS
        $E3A3: Unused. Puts Samus into morph ball on collision
        $E41B: Unused. Accelerates Samus Y speed up to 5, then sets her animation frame to 4 (invisible) if she's falling
        $E90E: RTS. Normal
    }
    $0A5C: Samus drawing handler
    {
        ...
        $EC14: Using elevator
        $EC1D: Samus received fatal damage. Used for suit pickup too?
    }
    $0A5E: Unused. Pointer to code to run (JMP from $90:F530). Only ever set to $F534 (RTS). Seems to be some kind of weird case debug function, checks for newly pressed Y on one of the controllers and forces a particular Samus state
    $0A60: Pointer to code to run (JMP from $90:E90F) (Once every frame, depending on 7E0A42) Looks like it handles controller input for Samus' poses.
    {
        $E90E: RTS
        $E913: Normal
        $E918: X-ray
    ...
    $0A64: Flags, 1 is Samus is currently grappled onto something, 2 is related to Draygon releasing Samus
    ...
    $0A68: Samus shine timer. Also used by crystal flash. X-ray too?
    ...
    $0A73: Samus visor palette index
    ...
    $0A7A..93: Demo input RAM
    {
        ...
        $0A8A: How many frames of input have been saved to bank B8. Yeeeaah. Enabled with 808002, otherwise never used
    ...
    $0A9A: Samus animation frame skip. 8000 means don't reset 0A96? (when changing poses?)
    ...
    $0AA2: Timer for shinesparks starting and stopping. Used when Samus enters the 'about to shinespark' pose, and when she hits a solid object.
    $0AA2?: Timer for Samus rising into position during crystal flash start
    ...
    $0AAA: 1 if HUD item changed this frame, otherwise 2
    ...
    $0AB4: Speed echo 2 X position (speed echo projectiles). Rate of acceleration for Shinespark crash echoes.
    $0AB6: Speed echo 3 X position (speed echo projectiles). X position of Samus during Shinespark crash projectile echoes
    $0AB8: Speed echo 0 Y position
    $0ABA: Speed echo 1 Y position
    $0ABC: Speed echo 2 Y position (speed echo projectiles). Index value for shinespark crash echo position?
    $0ABE: Speed echo 3 Y position (speed echo projectiles). Y position of Samus during Shinespark crash projectile echoes
    $0AC0: Speed echo 0 X speed
    $0AC2: Speed echo 1 X speed
    $0AC4: Speed echo 2 X speed (speed echo projectiles). Flag. Draw right projectile echoes for Shinespark crash. Set to #$0040 by crash routine
    $0AC6: Speed echo 3 X speed (speed echo projectiles). Flag. Draw left projectile echoes for Shinespark crash. Set to #$0040 by crash routine
    $0AC8: Samus' top half spritemap index
    $0ACA: Samus' bottom half spritemap index
    $0ACC: What kind of timer to use for 0A68? 2*this = JSL index for 91D72D.
    {
    ...
    $0ACE: Special Samus palette frame. Index used to get current Samus palette. Used for screw attack ($91:DA1B), speed boosting ($91:DA81), speed booster shine ($91:DAE0), shinespark ($91:DB45)
           Palette delay during hyper beam acquisition
           Palette index for x-ray visor
    $0AD0: Special Samus palette timer. Timer for 0ACE to get next Samus palette (only used for Speedbooster). Set to 0001 when starting to run, and then 0004 for each next palette
    ...
    $0AEC..F3: Low byte: Atmospheric graphics animation frames. High byte: Atmospheric graphics types
    {
        ...
        6: Dust due to landing (uses slots 2/3)
    ...
    $0B0C: Unused? Never seems to be written to except by ram clearing routines
    $0B0E: Ideal layer 1 Y position. [Samus' Y position] - [up/down scroller]
    $0B10: Samus previous X position.    Updated at end of scrolling routine ($90:94EC)
    $0B12: Samus previous X subposition. Updated at end of scrolling routine ($90:94EC)
    $0B14: Samus previous Y position.    Updated at end of scrolling routine ($90:94EC)
    ...
    $0B1C: Unused? Never seems to be written to except by ram clearing routines
    $0B1E: Unused? Never seems to be written to except by ram clearing routines
    $0B20: Used for bouncing as a ball when you land. Set to 0601 on the first bounce, and 0602 on the second bounce. STZ'd when changing poses or direction
    ...
    $0B2E: Samus Y speed. Samus Y velocity for jump in ending sequence ($8B:F651)
    ...
    $0B3A: Unused? Never seems written to except by ram clearing routines
    $0B3C: Has momentum flag(?) Set when pressed run whilst running, cleared when releasing forward. Flag to check Samus' speed counter? Reason why temporary blue suit cannot become permanent.
    $0B3E: Used for some weird animation counter thing for incrementing $0B3F
    $0B3F: Speed boost counter
    ...
    $0B5E: New projectile direction due to change of pose?
    $0B60: Used for the SBA attacks. Cleared by Spazer. Set to 0004 by Wave, but unused. Used as a rotation speed with Ice and Plasma
    $0B62: Samus' charge palette index. Includes pseudo screw attack
    $0B64..6D: Projectile X positions
    $0B6E..77: Bomb X positions
    $0B78..81: Projectile Y positions
    $0B82..8B: Bomb Y positions
    $0B8C..95: Projectile X subpositions
    $0B96..9F: Bomb X subpositions
    $0BA0..A9: Projectile Y subpositions
    $0BAA..B3: Bomb Y subpositions
    $0BB4..BD: Projectile X radii
    $0BBE..C7: Bomb X radii
    $0BC8..D1: Projectile Y radii
    $0BD2..DB: Bomb Y radii
    $0BDC..E5: Projectile X velocities (* 100h?)
    $0BE6..EF: Bomb X speeds
    $0BF0..F9: Projectile Y velocities
    ...
    $0C2C..35: Projectile damages
    $0C36..3F: Bomb damages
    ...
    $0C68..71: Projectile pre-instructions
    $0C72..7B: Bomb functions
    $0C7C..85: Projectile variables
    {
        Super missiles: Super missile indices. Top byte is a flag, linked if set. Bottom byte tells linked index
        Ice/plasma SBA: Angle of projectile from Samus (2pi / 100h radians clockwise, 0 = up)
        Wave SBA: Projectile speed
        Spazer SBA: ?
    }
    $0C86..8F: Bomb timers
    $0C90..99: Projectile trail timers
    $0C9A..A3: Bomb subspeeds
    ...
    $0CE2: X position of power bomb explosion (including when used to blow up Ceres station)
    $0CE4: Y position of power bomb explosion (including when used to blow up Ceres station)
    ...
    $0D16: X position of start of grapple beam
    $0D18: Y position of start of grapple beam
    $0D1A: X position of start of grapple beam previous frame
    $0D1C: Y position of start of grapple beam previous frame
    ...
    $0DA2: Absolute X distance Samus moved last frame + 1, pixels.
    $0DA4: Absolute X subdistance Samus moved last frame
    $0DA6: Absolute Y distance Samus moved last frame + 1, pixels.
    $0DA8: Absolute Y subdistance Samus moved last frame
    $0DAA: Distance Samus moved left (negative).    Used for projectile initial speed
    $0DAC: Subdistance Samus moved left (negative). Used for projectile initial speed
    $0DAE: Distance Samus moved right.              Used for projectile initial speed
    $0DB0: Subdistance Samus moved right.           Used for projectile initial speed
    $0DB2: Distance Samus moved up (negative).      Used for projectile initial speed
    $0DB4: Subdistance Samus moved up (negative).   Used for projectile initial speed
    $0DB6: Distance Samus moved down.               Used for projectile initial speed
    $0DB8: Subdistance Samus moved down.            Used for projectile initial speed
    ...
    $0DC6: Something for setting Samus prospective pose ($0A28/$0A2E)
    {
        0: No change
        1: Samus collided with block or solid enemy during down movement ("down movement" as in $90:9440)
        2: Samus didn't collide with a block or solid enemy during down movement
        3: Unused. Acts like 4
        4: Samus collided with block or solid enemy during up movement ("up movement" as in $90:93EC)
        5: Wall jump triggered
    }
    $0DC7: Something for $0DC6 if it is 1 or 2 (down movement)
    {
        If [$0DC6] = 1:
            0: Grounded
    ...
    $0DCE: Set if Samus collides with a solid enemy(Only read as a flag, written as a direction?)
    ...
    $0DE0: Debug invincibility. 7 = invincible
    $0DE2: Game options screen index. On-boot Nintendo logo timer
    {
        0:
        1: Loading options screen
        2: Fading in options screen
        3: Options screen
        4: Start game
        ...
        Ch: Fading out options screen to start game
    }

    $0DEA: Which item to decrement during crystal flash. 0 for missiles, 1 for super missiles, 2 for power bombs
    ...
    $0DEC:
    {
        ...
        Crystal flash timer for decrementing (super) missiles/power bombs (from 10 to 0)
    ...
    $0DEE:
    {
        ...
        Crystal flash related
        Draygon-escape previous d-pad input
    }
    $0DF0: Crystal flash related, set to Samus Y position
    $0DF2: Crystal flash related
...
$0E1C: Enemy index to shake (due to walljump whilst frozen)
...
$0E40: Damage multiplier for enemies
...
$0E4E: Number of enemies in the room
...
$1870: Enemy left border for enemy/projectile collisions
$1872: Enemy bottom border for enemy/projectile collisions
$1874: Enemy right border for enemy/projectile collisions
$1876: Enemy top border for enemy/projectile collisions
...
$187E: Samus right border when enemy is checking for collisions with Samus
$1880: Samus left border when enemy is checking for collisions with Samus
$1882: Samus bottom border when enemy is checking for collisions with Samus
$1884: Samus top border when enemy is checking for collisions with Samus
...
$198D..1C1E: Enemy projectile data. 18 enemy projectiles max
{
    $198D: Flag. 8000h enables enemy projectiles

    $1991: Enemy projectile index
    $1993: Enemy projectile initialisation parameter
...
$198D..1C1E: Non-gameplay use
{
    ...
    $1A49: Cinematic function timer (during intro at least)
    ...
    $1B3D..4A: Control icons during controller setting menu screen (Fire, Jump, Run, Select, Cancel, AimUp, AimDown. 0-6 = X, A, B, Select, Y, R, L)
...
$1F01..0D: Animated tiles object timers
...
{
    $7E:7000..9FFF: CRE tiles, or extended part of room tiles if [CRE bitset] & 4
}
$7E:2000..2FFF: Enemy BG2 tilemap. Transfer is flagged via $0E1E, also see extended tilemap format in "Enemy RAM.asm". Used by Kraid, Phantoon, Mother Brain, Crocomire
{
    Kraid:
    {
...
{
    $7E:4000..4FFF: X-rayed BG1 tilemap
...
$7E:7000..77FF: Enemy spawn data. 40h byte slots
{
    ...
    + 2: Cause of death? Cleared by main enemy routine when enemy AI is disabled, ([enemy flash timer] = 1 or [enemy frozen timer] = 1) before running death animation, 3 when power bomb killed, 4 when grapple killed
...
$7E:8000..87FF: Extra enemy RAM (40h bytes each)
{
    ...
    Botwoon:
    {
        $7E:8000: Botwoon initialisation timer. Initialised to 100h, Botwoon waits until it decrements to zero before first moving

        $7E:801E: Botwoon death flag
    }

    Draygon:
    {
        $7E:8000: Used for Draygon's direction when grabbing Samus. 0 = left, 1 = right
    }
...
$7E:8800..8FFF: Extra enemy RAM (40h bytes each). Includes variables for corpse rotting effect
{
    Draygon:
    {
        $7E:8802: Unused byte for topleft turret in Draygon's room
        $7E:8804: Death byte for bottomleft turret in Draygon's room (1 = no more turret fire)
        $7E:8806: Death byte for topright turret in Draygon's room (1 = no more turret fire)
        $7E:8808: Death byte for bottomright turret in Draygon's room (1 = no more turret fire)
    }
    ...
    Botwoon:
    {
        $7E:8818: Previous Botwoon health during enemy shot reaction

        $7E:881C: Botwoon palette data offset (should be 1E0h)

        $7E:8834: Botwoon spitting flag
        $7E:8836: Botwoon max health. Never read
        $7E:8838: Botwoon max health * 1/2
        $7E:883A: Botwoon max health * 1/4

        $7E:883E: Botwoon's body death flag
    }
}
...
$7E:9100..FF: Something to do with HDMA, used when Phantoon makes his swirl animation when he dies
...
$7E:C000..C1FF: Palettes, copied straight to CGRAM
{
	...
	$7E:C1A0..BF: Sprite palette 5 - most common sprites (item drops, smoke, explosions, bombs, power bombs, missiles, gates (wall part), water splashes, grapple beam)
	$7E:C1C0..DF: Sprite palette 6 - beams
...
$7E:C404: Current index for C000/C200
...
$7E:D7C0..DE1B: RAM that is saved to SRAM
{
    ...
    $7E:D910..13: Unused
    $7E:D914: Loading game state. Used in some initialisation routines to decide what to do. Used by gunship to decide if it should do the landing sequence, if so, sets this to 5 afterwards. Used by game over screen to decide where to reload Samus to.
    {
        0: Intro
        5: Main
...
$7E:EF78..F377: Sprite object RAM
{
    ...
    $7E:F178..F1F7: Sprite object X subpositions. Used only for Draygon related sprite object
    $7E:F1F8..F237: Sprite object Y positions

    $7E:F278..F2F7: Sprite object Y subpositions. Used only for Draygon related sprite object
...
$7F:0002..6401: Level data (layer 1). Word per block: ttttyxnnnnnnnnnn. t = block type (see "Special blocks.asm"), yx are Y/X flips, n = block index (into tile table, see $7E:A000)
